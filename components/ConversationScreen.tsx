...
import { convertFirestoreTimeStamToString, generateQueryGetMessage, transformMessage } from "../utils/getMessageInConversation"
...
const ConversationScreen = ({ conversation, messages }: { conversation: Conversation, messages: IMessage[] }) => {
    ...
    const showMessages = () => {
        // If front-end is loading messages behind the scenes, display messages from Next SSR (passed down from [id].tsx)
        if (messagesLoading) {
